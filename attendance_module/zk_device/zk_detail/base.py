...
class ZK(object):
    ...
    def get_attendance_json(self, dates_list=None, userIds=None, year=None, month=None):
        ...
        # if self.verbose: print ('self.verbose: users:- ',users)
        # attendances = []
        attendance_data, size = self.read_with_buffer(const.CMD_ATTLOG_RRQ)
        print(attendance_data,' ', size)
        if size < 4:
            # if self.verbose: print ("WRN: no attendance data")
            return []
        # total_size = unpack("I", attendance_data[:4])[0]
        ...
        while len(attendance_data) >= 40:
            ...
            if(attendance.user_id in userIds):
                ...
                if(year_month in str(timestamp)):
                    # userIds[str(attendance.user_id)][str(year_month)].add(("%s|%s"%(attendance.timestamp, attendance.punch)))
