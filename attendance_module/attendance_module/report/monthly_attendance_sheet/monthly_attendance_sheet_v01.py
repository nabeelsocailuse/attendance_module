...
def get_rows(employee_details: dict, filters: Filters, holiday_map: dict, attendance_map: dict) -> list[dict]:
    ...
    for employee, details in employee_details.items():
        ...
        if filters.summarized_view:
            ...
            # frappe.msgprint(frappe.as_json(attendance["total_absent"]))
            if not attendance:
                continue

            leave_summary = get_leave_summary(employee, filters)
            entry_exits_summary = get_entry_exits_summary(employee, filters)
            # frappe.msgprint(frappe.as_json(entry_exits_summary))
...
def get_attendance_status_for_detailed_view(
	...
	entry_exits_summary = get_entry_exits_summary(employee, filters)
	# frappe.msgprint(frappe.as_json(entry_exits_summary["total_early_exits"]))
	# frappe.msgprint(frappe.as_json(entry_exits_summary["total_late_entries"]))
	...
	for shift, status_dict in employee_attendance.items():
		...
		for row in attendance_values:
			row["total_present"] = summary.total_present + summary.total_half_days
			row["total_absent"] = summary.total_absent
			row["total_leaves"] = summary.total_leaves + summary.total_half_days
			row["total_late_entries"] = entry_exits_summary["total_late_entries"]
			row["total_early_exits"] = entry_exits_summary["total_early_exits"]

		# frappe.msgprint(frappe.as_json(attendance_values))
